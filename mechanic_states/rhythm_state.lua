...
function staff(musheet, ents_vect, state)
    local s = {}
    s.preview = {}
    s.preview.do_prev = true

    local notes = musheet.notes
    s.total_notes=#notes
    s.musheet = musheet
    s.speed = musheet.speed
    ...
    function s:done()
        if(not self.preview.do_prev)then
            ...
            -- here you can call a function on previous state
            -- state:duel_ended(dmg_taken, dmg_given, self.musheet)
        else
...
-- musheet must have a table with numbers from 0 to 3 inclusive. each number represents "a note"
-- it can have any number of notes 
-- number mapping: 0 left, 1 right, 2 up, 3 down
-- it also must have a "speed" member
-- example
--   musheet={notes={0,0,3,1,0,2}, speed=1.5}
function rhythm_state(musheet, prev_state)
    music(-1)
    local s={}
    local ents={}
    ...
    local staff = staff(musheet, ents, prev_state)
    add(ents, staff)
